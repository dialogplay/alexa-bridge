...
function sendReply(replyToId) {
  ...
  console.log("testreplytime " + currentTime())
  if (res.send) {
    res.send(reply);
    next();
  }
...
function createAlexaReply(activity) {
  console.log("createAlexaReplytime " + currentTime())
...
function createAlexaProgressReply(requestId) {
  console.log("createAlexaProgressReplytime " + currentTime())
  var reply = {
    "header": {
      "requestId": requestId
    },
...
function createAlexaSessionEndReply() {
  console.log("createAlexaSessionEndReplytime " + currentTime())
  var reply = {
    "version": "1.0",
    "response": {
      "outputSpeech": {
...
function botSays(activity) {
  console.log("botSaystime " + currentTime())
  ...
  if ((process.env.useMultipleResponses == 'false' || activity.inputHint == 'expectingInput') && activity.replyToId in responses) {
    ...
    console.log("before1createAlexaReplytime " + currentTime())
  ...
  console.log("before2createAlexaReplytime " + currentTime())
  if (activity.replyToId in msgParts) {
    ...
    console.log("before3createAlexaReplytime " + currentTime())
    msgParts[activity.replyToId] = [activity];
...
function alexaIntent(req, res, bot, next) {
  console.log("alexaIntenttime " + currentTime())
  let startTime = Date.now();
...
function alexaSays(req, res, bot, next) {
  console.log("alexaSaystime " + currentTime())
  ...
  if (req.body && req.body.request && req.body.request.type &&
    ...
    return next(new errors.InvalidArgumentError("Unhandled request type"));
  }
}
...
function startBridge() {
  console.log("startBridgetime " + currentTime())
  ...
  console.log("teststarttime" + currentTime())
